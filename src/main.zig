...
test "domain review lane" {
    const item = DomainReview{ .signal = 40, .slack = 49, .drag = 27, .confidence = 51 };
    try std.testing.expectEqual(@as(i32, 99), domainReviewScore(item));
    try std.testing.expect(std.mem.eql(u8, domainReviewLane(item), "hold"));
}
